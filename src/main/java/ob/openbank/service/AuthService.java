...
public class AuthService {
  ...
  public void validateAndSendForm(String clientId, HttpServletResponse response)
      throws IOException {
  ...
  public Long getNextSequenceValue() {
    // 시퀀스의 다음 값 가져오기
    return ((BigDecimal) entityManager.createNativeQuery("SELECT seq_no_seq.NEXTVAL FROM dual")
        .getSingleResult()).longValue();
  }

  @Transactional
  public AccessTokenResponseDTO generateAccessToken(String grantType, String code,
...
//                .seq_no(formattedSeqNo)
  ...
  public String authorize2(String responseType, String clientId, String redirectUri, String scope,
      String state, String ci, String customerId) throws Exception {
    try {
      OpenbankingAuthentication auth = authenticationRepository.findById(ci)
          .orElse(new OpenbankingAuthentication());
      ...
      return redirectUri + "?code=" + authorizationCode + "&state=" + state + "&customer_id="
          + customerId;
    } catch (Exception e) {
  ...
  public RequestAuthCodeResponseDTO generateAuthCode(String accessToken) {
    ...
    String authCode = UUID.randomUUID()
        .toString();  // Placeholder for actual auth code generation logic
